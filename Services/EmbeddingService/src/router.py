...
from src.schemas import (
    ...
    BatchVectorizeRequest, DeleteRequest
)
...
async def vectorize_and_store(request: VectorizeRequest):
    try:
        point_id, dimensions, collection = await run_in_threadpool(
            embedding_service.vectorize_and_store,
            request.text,
            request.metadata,
...
async def vectorize_batch(request: BatchVectorizeRequest):
    try:
        count, collection = await run_in_threadpool(
            embedding_service.vectorize_batch,
            request.items,
            request.collection_name
        )
...
async def delete_document(request: DeleteRequest):
    try:
        collection = await run_in_threadpool(
            embedding_service.delete_by_filter,
            request.source_id,
            request.tenant_id,
            request.type,
            request.collection_name
